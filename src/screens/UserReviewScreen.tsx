...
const UserReviewScreen = ({ navigation }: Props) => {
  ...
  const _handleComplete = () => {
    ...
    setWarningProps({
      ...
      dismissable: true,
      dialogTitle: '사용자 리뷰 제출',
      dialogContent: '사용자 리뷰를 이대로 제출합니다. 제출한 리뷰는 수정하거나 재작성하실 수 없습니다. 그래도 진행하시겠습니까?',
      okText: '제출합니다',
      dismissText: '아니오',
    });
  };
  ...
  const renderHeader = () => (
    <SwitchStackHeader appearance={appearance} border>
      ...
      <Body flex={4}>
        <HeaderTitle tintColor={getThemeColor('text', appearance)}>사용자 리뷰 남기기</HeaderTitle>
      </Body>
      <Right>
