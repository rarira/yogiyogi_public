...
const AppleSignUp = ({ snackbarDispatch, navigation }: Props) => {
  ...
  const appleUserId = `Apple_${thirdPartyId.id}`;
  const appleUserEmail = thirdPartyId.email ?? '';
  console.log(authInfo, appleUserId, appleUserEmail, kakaoPw);
      ...
      switch (e.code) {
        ...
        case 'NotAuthorizedException':
          snackbarDispatch({
            ...
            sideEffect: () => resetForm({ values: { email: '' } }),
            snackbarType: 'error',
          });
          return;
        default:
  ...
  return (
    <>
      ...
      <AuthContainer
        headline="애플 이메일 인증"
        subHeadline={`아래 ${thirdPartyId.isPrivateEmail ? '가상 ' : ''}이메일 주소로 인증 코드를 발송합니다. ${
          thirdPartyId.isPrivateEmail ? '본인의 애플 로그인 계정 이메일 주소를 확인하세요' : ''
        }`}
      >
        <Formik
          ...
          initialValues={{ email: appleUserEmail }}
      ...
      <MyDialogContainer visible={errorVisible} onDismiss={_handleErrorOnDismiss}>
        <MyDialogTitle>애플 로그인 중단</MyDialogTitle>
        <Dialog.Content>
          <DialogContentText text="애플 로그인을 이용하지 않습니다." />
          <DialogContentText text="로그인 화면으로 돌아갑니다" />
        </Dialog.Content>
        <Dialog.Actions>
          <ThemedButton onPress={_handleErrorOnDismiss}>취소</ThemedButton>
          <ThemedButton onPress={exitAppleLogin} color={getThemeColor('accent', appearance)}>
            로그인 돌아가기
          </ThemedButton>
        </Dialog.Actions>
      </MyDialogContainer>
      ...
      {/* <MyDialogContainer visible={emailVisible} onDismiss={_handleEmailOnDismiss} dismissable={false}>
...
export default withNavigation(AppleSignUp);
