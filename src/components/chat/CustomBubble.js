import { Clipboard, StyleSheet, Text, TouchableWithoutFeedback, View, ViewPropTypes } from 'react-native';
import { MessageImage, MessageText, MessageVideo, QuickReplies, Time } from 'react-native-gifted-chat';
...
const styles = {
  left: StyleSheet.create({
    ...
    wrapper: {
      borderRadius: 15,
      backgroundColor: theme.chatColor.leftBubbleBackground,
      marginRight: 60,
      minHeight: 20,
  ...
  right: StyleSheet.create({
    ...
    wrapper: {
      borderRadius: 15,
      backgroundColor: theme.chatColor.defaultBlue,
      marginLeft: 60,
      minHeight: 20,
      justifyContent: 'flex-end',
    },
  ...
  content: StyleSheet.create({
    tick: {
      fontSize: 10,
      backgroundColor: theme.chatColor.backgroundTransparent,
      color: theme.chatColor.white,
    },
    tickView: {
      flexDirection: 'row',
      marginRight: 10,
...
export default class CustomBubble extends React.Component {
  ...
  renderTicks() {
    const { currentMessage, renderTicks, user, extraData } = this.props;
    // console.log(currentMessage);
    if (renderTicks && currentMessage) {
      return renderTicks(currentMessage, extraData);
    }
    ...
    if (this.props.renderUsernameOnMessage && currentMessage) {
      ...
      return (
        <View style={styles.content.usernameView}>
          <Text style={[styles.content.username, this.props.usernameStyle]}>~ {currentMessage.user.name}</Text>
        </View>
      );
    }
    return null;
  ...
  render() {
    const { position, containerStyle, wrapperStyle, bottomContainerStyle, partnerState, extraData } = this.props;
    return (
      <View style={[styles[position].container, containerStyle && containerStyle[position]]}>
        <View style={[styles[position].wrapper, wrapperStyle && wrapperStyle[position], this.styledBubbleToNext(), this.styledBubbleToPrevious()]}>
...
//# sourceMappingURL=CustomBubble.js.map
